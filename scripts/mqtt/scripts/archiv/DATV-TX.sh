...
VBITRATE=$(( 1000 * $TSBITRATE * 45 / 100 / 1000 - $ABITRATE))
fi
# Up to 66K
...
VBITRATE=$(( 1000 * $TSBITRATE * 89 / 100 / 1000 - $ABITRATE))
fi
# Above 4000K leads to invalid parameter check
if (( $(bc <<<"$SR > 3000") )); then
VBITRATE=$(( 1000 * $TSBITRATE * 93 / 100 / 1000 - $ABITRATE))
fi
...
if [ "$FEC" == "2/3" ]; then
VBITRATE=$(( $VBITRATE * 118 / 100 ))
fi
if [ "$FEC" == "3/4" ]; then
VBITRATE=$(( $VBITRATE * 119 / 100 ))
fi
if [ "$FEC" == "5/6" ]; then
VBITRATE=$(( $VBITRATE * 121 / 100 ))
fi
if [ "$FEC" == "8/9" ]; then
VBITRATE=$(( $VBITRATE * 121 / 100 ))
fi
if [ "$FEC" == "9/10" ]; then
VBITRATE=$(( $VBITRATE * 122 / 100 ))
fi
...
# Raus  -shortest -shortest_buf_duration 1 VVC -dts_delta_threshold 1 -dts_error_threshold 1
#
...
-i "$AUDIODEVICE" -vcodec $CODEC -s $IMAGESIZE -b:v "$VBITRATE"K -r $FPS -g $KEYHARD -b_ref_mode 0 -bf 0 -cbr true -rc-lookahead 10 -no-scenecut 1 -zerolatency 1 -preset slow \
...
-vcodec $CODEC -s $IMAGESIZE -b:v "$VBITRATE"K -r $FPS -g $KEYHARD -b_ref_mode 0 -bf 0 -cbr true -rc-lookahead 10 -no-scenecut 1 -zerolatency 1 -preset slow \
-acodec $AUDIOCODEC -ac $AUDIO -b:a "$ABITRATE"K \
...
# Nvidia-Driver bug: -bf 0 switches of check of B-Frames  File play: -re    start /B /$PRIO 
# Raus  -shortest -shortest_buf_duration 1 VVC -dts_delta_threshold 1 -dts_error_threshold 1
