...
bool waterfall;
bool spectrum;
float FreqGainCal = 0.0; // 5.0 per dB
...
int CreateButton(int MenuIndex, int ButtonPosition)
{
  ...
  else if (MenuIndex == 6)   // Scan Width Menu
  {
    ...
    if ((ButtonPosition == 11) || (ButtonPosition == 12))  // Bottom 2 buttons
    {
      x = normal_xpos;
      y = 480 - ((ButtonPosition - 4) * 60);
      w = normal_width;
...
void CalculateMarkers()
{
  ...
  if (Range20dB == false)
  {
    markerlev = ((float)(410 - markery) / 5.0) - 120.0 - (float)(ScaleShift + atten);  // in dB for a 0 to -80 screen
  }
  else
  {
    markerlev = ((float)(410 - markery) / 20.0) - 40.0 + (float)(BaseLine20dB - atten);  // in dB for a BaseLine20dB to 20 above screen
  }
  ...
  switch (button)
  {
    case 0:                                                                         // On entry to Sig Gen menu
      ...
      if (CheckExpressRunning() != 0)        // DATV Express Server not running
      {
        ...
        // Change menu to say firmware loading
...
void *WaitButtonEvent(void * arg)
{
  ...
  for (;;)
  {
    ...
    if (CurrentMenu == 13)  // Other Apps Menu
    {
...
void Define_Menu5()                                          // Mode Menu
{
  ...
  AddButtonStatus(button, "Other Apps^Menu", &Blue);
...
void Start_Highlights_Menu5()
{
  ...
  {
    if ((spectrum == true) && (waterfall == false))  // Classic SA
    {
      SetButtonStatus(ButtonNumber(CurrentMenu, 2), 1);
    }
    else
    {
      if ((spectrum == true) && (waterfall == true))  // mix
      {
        SetButtonStatus(ButtonNumber(CurrentMenu, 5), 1);
      }
      else if ((spectrum == false) && (waterfall == true))  // waterfall
      {
        SetButtonStatus(ButtonNumber(CurrentMenu, 4), 1);
      }
    }
...
void Define_Menu6()                                           // Span Menu
{
  ...
  AddButtonStatus(button, "30^MHz", &Blue);
...
void Define_Menu13()                                          // Waterfall Config Menu
{
  int button = 0;
...
void DrawYaxisLabels()
{
  ...
  if ((Range20dB == false) && (waterfall == false))       // Full range spectrum
  {
    for (i = 0; i <= 8; i++)
    {
      snprintf(caption, 15, "%d dBm", ((i + 4) * -10) - atten - ScaleShift);
  ...
  else if ((Range20dB == true) && (waterfall == false))  // 20 dB range spectrum
  {
    for (i = 0; i <= 4; i++)
    {
      snprintf(caption, 15, "%d dBm", ((i + 4) * -5) - atten + BaseLine20dB );
  ...
  else if ((spectrum == true) && (waterfall == true))   // Mix
  {
    for (i = 0; i <= 2; i++)
    {
      snprintf(caption, 15, "%d dBm", (((i * 4) + 4) * -10) - atten - ScaleShift);
      Text(10, 462 - (50 * i), caption, font_ptr, 0, 0, 0, 255, 255, 255);
    }
...
void DrawSettings()
{
  ...
  switch (span)
  {
    ...
    case 20480:
      ...
      break;
    case 30720:
      snprintf(SpanText, 63, "Span 30 MHz");
      break;
  }
  ...
  snprintf(FreqText, 63, "Centre %5.2f MHz    ", centrefreq / 1000.0);
  ...
  do
  {
    ...
    if ((samodeauto == true) && (LimeStarted == true))
    {
      switch (samode)
      {
        ...
        case 8:
        case 9:
          printf("\nExiting because mode switch is not slected to SA SDR\n\n");
          cleanexit(170);
          usleep(10000);
          break;
      } 
