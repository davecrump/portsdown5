...
#define PATH_XYCONFIG "/home/pi/portsdown/configs/pm_config.txt"
...
void do_snapcheck()
{
  ...
  first_paint = true;                                 // repaint meter pointer if static
}
  ...
  switch (button)
  {
    case 2:                                                       // Start Freq
      ...
      else
      {
        ...
        {
          snprintf(InitText, 31, "%d.%03d", div_1000.quot, div_1000.rem);
        }
        else
        {
          ...
          else
          {
            ...
            {
              snprintf(InitText, 31, "%d", div_1000.quot);
            }
          }
        }
      }
    ...
    case 4:                                                       // Ref Level
      ...
      {
        snprintf(InitText, 31, "%d", reflevel);
      }
    ...
    case 7:                                                       // Set Normalise Level
      ...
      // Define initial value 
      snprintf(InitText, 31, "%d", normlevel);
...
void ChangeRange(int button)
{
  ...
  pthread_mutex_lock(&text_lock);
  Text(120, 180, MeterUnits, font_ptr, 0, 0, 0, 63, 255, 63);
  pthread_mutex_unlock(&text_lock);
}
...
void ChangeSensor(int button)
{
    switch (button)
    {
      ...
      case 4:                                         // ad8318-k for KeyLimePi
        strcpy(MeterSensor, "ad8318-k");
        SetConfigParam(PATH_XYCONFIG, "metersensor", "ad8318-k");
      break;
      case 5:
        strcpy(MeterSensor, "bolo");
        SetConfigParam(PATH_XYCONFIG, "metersensor", "bolo");
        break;
      case 6:
...
void Define_Menu5()  // Mode (XY and Power)
{
  ...
  AddButtonStatus(button, "Sweeper", &Blue);
  AddButtonStatus(button, " ", &Green);
...
void Define_Menu9()  // Power Meter Sensor
{
  ...
  AddButtonStatus(button, "Bolometer", &Blue);
  AddButtonStatus(button, "Bolometer", &Green);
...
void Start_Highlights_Menu9()
{
  ...
  else if (strcmp(MeterSensor, "ad8318-k") == 0)
  {
    SetButtonStatus(ButtonNumber(9, 2), 0);
    SetButtonStatus(ButtonNumber(9, 3), 0);
  ...
  else if (strcmp(MeterSensor, "bolo") == 0)
  {
    SetButtonStatus(ButtonNumber(9, 2), 0);
    SetButtonStatus(ButtonNumber(9, 3), 0);
    SetButtonStatus(ButtonNumber(9, 4), 0);
...
void DrawYaxisLabels()
{
  ...
  for (i = 0; i <= 8; i++)
  {
    snprintf(caption, 15, "%d dBm", ((i) * -10));
...
void DrawSettings()
{
  ...
  if ((stopfreq > 0) && (stopfreq < 10000000))  // valid and less than 10 GHz
  {
    ...
    Text(440, line1y, DisplayText, font_ptr, 0, 0, 0, 255, 255, 255);
  }
...
void DrawMeterSettings()
{
  ...
  snprintf(DisplayText, 127, "Sensor %s   Attenuator %0.1f dB   Cal Factor %d%%", MeterSensor, Meteratten - (float)atten, (int)Metercalfactor);

  Text(100, line1y, DisplayText, font_ptr, 0, 0, 0, 255, 255, 255);

  if (strcmp(MeterTitle, "-") != 0)
  {
    TextMid(350, titley, MeterTitle, &font_dejavu_sans_22, 0, 0, 0, 255, 255, 255);
  }
}
...
void Draw5MeterLabels(float LH_Value, float RH_Value)
{
  ...
  if (abs(LH_Value - RH_Value) < 10.0)  // Display DP
  {
    ...
    TextMid(120, 345, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);

    snprintf(labeltext, 14, "%0.1f", LH_Value + (RH_Value -LH_Value) / 5);
    TextMid(195, 395, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);

    snprintf(labeltext, 14, "%0.1f", LH_Value + 2 * (RH_Value -LH_Value) / 5);
    TextMid(295, 430, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);

    snprintf(labeltext, 14, "%0.1f", LH_Value + 3 * (RH_Value -LH_Value) / 5);
    TextMid(405, 430, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);

    snprintf(labeltext, 14, "%0.1f", LH_Value + 4 * (RH_Value -LH_Value) / 5);
    TextMid(505, 395, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);

    snprintf(labeltext, 14, "%0.1f", RH_Value);
    TextMid(580, 345, labeltext, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  else
  //No DP
  {
...
void *MeterMovement(void * arg)
{
  ...
  while((false == *exit_requested))
  {
    ...
    if ((meter_deflection != current_meter_deflection) || (first_paint == true))
    {
      // Physics
      ...
      if (((meter_deflection - current_meter_deflection) != 0) || (first_paint == true))                                // Only draw if needed
      {
...
void ShowdBm(float dBm)
{
  ...
  pthread_mutex_lock(&text_lock);
  Text(110, 80, dBmtext, font_ptr, 0, 0, 0, 255, 255, 255);
  pthread_mutex_unlock(&text_lock);
}
...
void ShowmW(float mW)
{
  ...
  {
    if ((strcmp(MeterSensor, "bolo") == 0) && (adcreturn < 5))
    {
      // And overwrite previous text
      rectangle(400, 80, 199, 50, 0, 0, 0);
      snprintf(mWtext, 20, "%d", adcreturn);
    }
    else
    {
  ...
  pthread_mutex_lock(&text_lock);
  Text(400, 80, mWtext, font_ptr, 0, 0, 0, 255, 255, 255);
  pthread_mutex_unlock(&text_lock);
}
...
void Showraw(int raw)
{
  ...
  //setBackColour(0, 0, 0);
  pthread_mutex_lock(&text_lock);
  Text(110, 80, rawtext, font_ptr, 0, 0, 0, 255, 255, 255);
  pthread_mutex_unlock(&text_lock);
...
void CheckWithinRange(int advalue)
{
  ...
  if (strcmp(MeterSensor, "ad8318-5") == 0)
  {
    ...
    {
      pthread_mutex_lock(&text_lock);
      Text(110, 140,"Under-range", font_ptr, 0, 0, 0, 255, 63, 36);
      pthread_mutex_unlock(&text_lock);
    }
    else
    {
      pthread_mutex_lock(&text_lock);
      Text(110, 140,"Under-range", font_ptr, 0, 0, 0, 0, 0, 0);
      pthread_mutex_unlock(&text_lock);
    }
    if (advalue < ad8318_5_overrange)
    {
      pthread_mutex_lock(&text_lock);
      Text(450, 140,"Over-range", font_ptr, 0, 0, 0, 255, 63, 36);
      pthread_mutex_unlock(&text_lock);
    }
    else
    {
      pthread_mutex_lock(&text_lock);
      Text(450, 140,"Over-range", font_ptr, 0, 0, 0, 0, 0, 0);
      pthread_mutex_unlock(&text_lock);
    }
  }
  ...
  if (strcmp(MeterSensor, "ad8318-k") == 0)
  {
    if (advalue > ad8318_k_underrange)
    {
      pthread_mutex_lock(&text_lock);
      Text(450, 140,"Over-range", font_ptr, 0, 0, 0, 255, 63, 36);
      pthread_mutex_unlock(&text_lock);
  ...
  if (strcmp(MeterSensor, "bolo") == 0)
  {
    if (adcreturn > 4640) //  to bolo
    {
      pthread_mutex_lock(&text_lock);
      Text(450, 140,"Over-range", font_ptr, 0, 0, 0, 255, 63, 36);
      pthread_mutex_unlock(&text_lock);
...
void *PanelMonitor_thread(void *arg)
{
  ...
  do
  {
    adcreturn = analogRead (AD_BASE + 5);  // pin =  4 sets diff mode on 2 and 3 
    ...
    if (samodeauto == true)
    {
      ...
      if (modechange == true) // Wait 200ms and check again before exiting
      {
        usleep(200000);
      ...
      if ((strcmp(MeterSensor, "ad8318-k") != 0)  && (samode == 3))  // front panel 8318, but not correct sensor
      {
        ChangeSensor(4);
      }
      if ((strcmp(MeterSensor, "bolo") != 0)  && (samode == 4))  // front panel bolo, but not correct sensor
      {
        ChangeSensor(5);
      }
      modechange = false;
...
int main(int argc, char **argv)
{
  ...
  int previous_adcreturn = -1;  // 
  int nextwebupdate = 10;
  char response[63];
  float smoothedadcreturn = 0; 
  float power_dBm;
  float power_mW;
  ...
  while(app_exit == false)
  {
    if (Meter == true)
    {
      ...
      if (strcmp(MeterSensor, "bolo") == 0)
      {
        smoothedadcreturn = 0.05 * (float)adcreturn + 0.95 * smoothedadcreturn;
        if (smoothedadcreturn < 1)
        {
          smoothedadcreturn = 1;
          power_dBm = -90;
        }
        ...
        {
          power_dBm = 10.0 * log10(10.0 * smoothedadcreturn / bolo_cal);
        }
      }

      // Correct for front panel attenuator
      power_dBm = power_dBm - atten;
      ...
      if (((strcmp(MeterSensor, "bolo") != 0) && (rawvalue != previous_rawvalue))
       || ((strcmp(MeterSensor, "bolo") == 0) && (adcreturn != previous_adcreturn)))         // Update numeric display
      {
        if (strcmp(MeterSensor, "raw") == 0)
        {
          Showraw(rawvalue);
        ...
        {
          ShowdBm(power_dBm);
          if (adcreturn < 5)  // show negative values for zeroing
          {
            ShowmW(10.0 * (float)adcreturn / bolo_cal);
          }
          else
          {
            ShowmW(power_mW);
          }
          previous_adcreturn = adcreturn;
