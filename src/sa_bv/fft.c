...
void *fft_thread(void *arg)
{
  ...
  //uint64_t pre_fft = monotonic_ms();
  //uint64_t post_fft = monotonic_ms();
  ...
  while((false == *exit_requested))   // Go around once for each Lime input buffer fill?
  {
    ...
    {
      if (band >= 5 )     // Spectrum does not need to be reversed for bands 5 - 8
      {
      ...
      else                // Reverse Spectrum for lower bands
      {
    ...
    for (i = 0; i < FFT_SIZE; i++)
    {
      ...
      // Correct for Lime Gain and frequency
      fft_scaled_data[i] = fft_scaled_data[i] + 5 * FreqGainCal;

      if (Range20dB) // Range20dB
      {
        fft_scaled_data[i] = fft_scaled_data[i] - 5 * (75 + BaseLine20dB);  
