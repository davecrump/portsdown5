...
//LMS_Reset(device);
  ...
  else                                       // 1 - 20 MHz bandwidth
  {
    ...
    printf("Set init sample rate:     LMS_SetSampleRate(device, %.1f, 1)\n", bandwidth);
    LMS_SetSampleRate(device, bandwidth, 1);
  }
...
//  CalFreq = frequency_actual_rx;
  ...
  if (strcmp(device_info->deviceName, "LimeSDR Micro") == 0)
  {
    LMS_SetAntenna(device, LMS_CH_RX, 0, 2); //LNA_H
  }
...
if (false)  // Set true fro antenna switch testing
{
  ...
  printf("Antenna requested = %d, LMS_SetAntenna returns %d, Antenna %d %s selected\n", AntRequested, SetResult, AntFound, antlist[AntFound]);
    ...
    if (NewSpan == true)
    {
      ...
      {
        printf("Amend sample rate:        LMS_SetSampleRate(device, %.1f, 1)\n", bandwidth);
        LMS_SetSampleRate(device, bandwidth, 4);
      }
