...
static DVB2FrameFormat S2Format;
...
#define MAX_SYMBOLS_REMAINING 12
	...
	if (S2Format.constellation == M_QPSK)
	{
		...
		for (size_t i = 0; i < (size_t)DVBS2Length; i++)
		{
			...
			{
			case 23169:
				dibit = (FrameS2IQ[i].im == 23169) ? 0 : 1;
				break;
			case -23169:
		...
		//fprintf(stderr,"DVBS2Length %d index %d shift %d\n",DVBS2Length,index,shift);
		for (size_t i = 0; i < Remaining; i++)
		{
			short dibit;
	...
	if (S2Format.constellation == M_8PSK)
	{
		for (size_t i = 0; i < Remaining; i++)
		{
			package |= RemainingSymbolTab[i] << (shift * 3 + 4);
			shift++;
			if (shift == 4)
			{
				dvbs2_symbols_map[index] = package;
		...
		for (size_t i = 0; i < (size_t)DVBS2Length; i++)
		{
			...
			{
			case 23169:
				dibit = (FrameS2IQ[i].im == 23169) ? 0 : 5;
				break;
			case -23169:
				dibit = (FrameS2IQ[i].im == 23169) ? 6 : 3;
				break;
			case 32767:
				dibit = 1;
				break;
			case -32767:
				...
				break;	
			case 0:
				dibit = (FrameS2IQ[i].im == 32767) ? 4 : 7;
				break;	
