...
#define PATH_SCONFIG "/home/pi/portsdown/configs/system_config.txt"//#define PATH_PPRESETS "/home/pi/rpidatv/scripts/portsdown_presets.txt"
...
#define PATH_CAL "/home/pi/portsdown/configs/siggencal.txt"
...
bool touch_inverted = false;           // true if screen mounted upside down
...
void ReadSavedState()
{
  ...
  // Read current Display Type from System Config File
...
void SaveState()
{
  ...
  snprintf(value, 12, "%ld", DisplayFreq);
...
void ReadWebControl()
{
  ...
  GetConfigParam(PATH_SCONFIG, "webcontrol", WebControlText);
...
void LimeInfo()
{
  ...
  //clearScreen(0, 0, 0);
  Text(wscreen/12, hscreen - 1 * linepitch, "Lime Firmware Information", font_ptr, 0, 0, 0, 255, 255, 255);
  ...
  while (fgets(response, 50, fp) != NULL)
  {
    ...
    {
      Text(wscreen/12, hscreen - (1.2 * line + 2) * linepitch, response, font_ptr, 0, 0, 0, 255, 255, 255);
    }
    line = line + 1;
  }
  ...
  /* close */
  pclose(fp);
  Text(wscreen/12, 1.2 * linepitch, "Touch Screen to Continue", font_ptr, 0, 0, 0, 255, 255, 255);
}
...
void DrawButton(int ButtonIndex)
{
  ...
  {                                                 // 2 lines
    ...
    TextMid(Button->x+Button->w/2, Button->y+Button->h*11/16, line1, &font_dejavu_sans_20, Button->Status[Button->NoStatus].Color.r,
                Button->Status[Button->NoStatus].Color.g, Button->Status[Button->NoStatus].Color.b, 255, 255, 255);	
    TextMid(Button->x+Button->w/2, Button->y+Button->h*3/16, line2, &font_dejavu_sans_20, Button->Status[Button->NoStatus].Color.r,
                Button->Status[Button->NoStatus].Color.g, Button->Status[Button->NoStatus].Color.b, 255, 255, 255);	
    ...
    if ((((CurrentMenu == 1) && (ButtonIndex == 5)) || (CurrentMenu == 4)) && (Button->NoStatus == 0))
    {
      ...
      TextMid(Button->x+Button->w/2, Button->y+Button->h*3/16, line2, &font_dejavu_sans_20, Button->Status[1].Color.r,
        Button->Status[1].Color.g, Button->Status[1].Color.b, 255, 255, 255);	
    }
  }
  else                                              // One line only
  {
    ...
    {
      TextMid(Button->x+Button->w/2, Button->y+Button->h/2, label, &font_dejavu_sans_28, Button->Status[Button->NoStatus].Color.r,
                Button->Status[Button->NoStatus].Color.g, Button->Status[Button->NoStatus].Color.b, 255, 255, 255);
    }
    else if (CurrentMenu == 12)  // Keyboard
    {
      TextMid(Button->x+Button->w/2, Button->y+Button->h/2 - hscreen / 64, label, &font_dejavu_sans_28, Button->Status[Button->NoStatus].Color.r,
                Button->Status[Button->NoStatus].Color.g, Button->Status[Button->NoStatus].Color.b, 255, 255, 255);
    }
    else // fix text size at 20
    {
      TextMid(Button->x+Button->w/2, Button->y+Button->h/2, label, &font_dejavu_sans_20, Button->Status[Button->NoStatus].Color.r,
                Button->Status[Button->NoStatus].Color.g, Button->Status[Button->NoStatus].Color.b, 255, 255, 255);
    }
  }
}
...
void ShowFreq(uint64_t DisplayFreq)
{
  ...
  char df01text[31], df02text[31], df03text[31], df04text[31] ,df05text[31];
  char df06text[31], df07text[31], df08text[31], df09text[31] ,df10text[31];
  char df11text[31];

  df01 = DisplayFreq/10000000000;
  snprintf(df01text, 11, "%ld", df01);
  RemFreq = DisplayFreq - df01 * 10000000000;

  df02 = RemFreq/1000000000;
  snprintf(df02text, 12, "%ld", df02);
  RemFreq = RemFreq - df02 * 1000000000;

  df03 = RemFreq/100000000;
  snprintf(df03text, 13, "%ld", df03);
  RemFreq = RemFreq - (df03 * 100000000);

  df04 = RemFreq/10000000;
  snprintf(df04text, 14, "%ld", df04);
  RemFreq = RemFreq - df04 * 10000000;

  df05 = RemFreq/1000000;
  snprintf(df05text, 15, "%ld", df05);
  RemFreq = RemFreq - df05 * 1000000;

  df06 = RemFreq/100000;
  snprintf(df06text, 16, "%ld", df06);
  RemFreq = RemFreq - df06 * 100000;

  df07 = RemFreq/10000;
  snprintf(df07text, 17, "%ld", df07);
  RemFreq = RemFreq - df07 * 10000;

  df08 = RemFreq/1000;
  snprintf(df08text, 18, "%ld", df08);
  RemFreq = RemFreq - df08 * 1000;

  df09 = RemFreq/100;
  snprintf(df09text, 20, "%ld", df09);
  RemFreq = RemFreq - df09 * 100;

  df10 = RemFreq/10;
  snprintf(df10text, 20, "%ld", df10);
  RemFreq = RemFreq - df10 * 10;

  df11 = RemFreq;
  snprintf(df11text, 21, "%ld", df11);

  // Display Text
  if (DisplayFreq >= 10000000000)
  {
    TextOnly(0.06*wscreen, vpos*hscreen, df01text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 1000000000)
  {
    TextOnly(0.14*wscreen, vpos*hscreen, df02text, font_ptr, 0, 0, 0, 255, 255, 255);
    TextOnly(0.20*wscreen, vpos*hscreen, ",", font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 100000000)
  {
    TextOnly(0.24*wscreen, vpos*hscreen, df03text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 10000000)
  {
    TextOnly(0.32*wscreen, vpos*hscreen, df04text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 1000000)
  {
    TextOnly(0.40*wscreen, vpos*hscreen, df05text, font_ptr, 0, 0, 0, 255, 255, 255);
    TextOnly(0.458*wscreen, vpos*hscreen, ".", font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 100000)
  {
    TextOnly(0.495*wscreen, vpos*hscreen, df06text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 10000)
  {
    TextOnly(0.575*wscreen, vpos*hscreen, df07text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 1000)
  {
    TextOnly(0.655*wscreen, vpos*hscreen, df08text, font_ptr, 0, 0, 0, 255, 255, 255);
    TextOnly(0.715*wscreen, vpos*hscreen, ",", font_ptr, 0, 0, 0, 255, 255, 255);
  }
  if (DisplayFreq >= 100)
  {
    TextOnly(0.75*wscreen, vpos*hscreen, df09text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  TextOnly(0.83*wscreen, vpos*hscreen, df10text, font_ptr, 0, 0, 0, 255, 255, 255);
  TextOnly(0.91*wscreen, vpos*hscreen, df11text, font_ptr, 0, 0, 0, 255, 255, 255);
...
void ShowLevel(int DisplayLevel)
{
  ...
  vpos = 0.14;
  TextOnly(0.02*wscreen, vpos*hscreen, dl01text, font_ptr, 0, 0, 0, 255, 255, 255);
  if ((DisplayLevel <= -100) || (DisplayLevel >= 100))
  {
    TextOnly(0.11*wscreen, vpos*hscreen, dl02text, font_ptr, 0, 0, 0, 255, 255, 255);
  }
  TextOnly(0.19*wscreen, vpos*hscreen, dl03text, font_ptr, 0, 0, 0, 255, 255, 255);
  if(strcmp(osc, "pluto5") != 0)
  {
    TextOnly(0.25*wscreen, vpos*hscreen, ".", font_ptr, 0, 0, 0, 255, 255, 255);
    TextOnly(0.29*wscreen, vpos*hscreen, dl04text, font_ptr, 0, 0, 0, 255, 255, 255);
    TextOnly(0.37*wscreen, vpos*hscreen, "dBm", font_ptr, 0, 0, 0, 255, 255, 255);
  }
...
void ShowAtten()
{
    ...
    && (strcmp(osc, "pluto5") != 0) && (AttenIn == 1))  // do Attenuator text first
  {
    ...
    strcat(LevelText, " dB");
    Text(hpos*wscreen, vpos*hscreen, LevelText, font_ptr, 0, 0, 0, 255, 255, 255);
  }
...
void ShowOPFreq()
{
  ...
  if ((strcmp(osc, "elcom") == 0) && (CurrentMenu == 11))
  {
      ...
      snprintf(FreqString, 12, "%ld", OutputFreq);
      strcat(OPFreqText, FreqString);
      Text(hpos*wscreen, vpos*hscreen, OPFreqText, font_ptr, 0, 0, 0, 255, 255, 255);
  }

  if ((strcmp(osc, "slo") == 0) && (CurrentMenu == 11))
  {
      strcpy(OPFreqText, "Output Freq = ");
      snprintf(FreqString, 12, "%ld", OutputFreq * 4);
      strcat(OPFreqText, FreqString);
      Text(hpos*wscreen, vpos*hscreen, OPFreqText, font_ptr, 0, 0, 0, 255, 255, 255);
  }
...
void killExpressServer()
{
  system("sudo killall -9 express_server &");
}
...
void adf4153On()
{
  ...
  printf("R Counter = %d\n", r_counter);
  printf("Calculated Frequency = %ld\n", calculated_frequency);
...
void ShowTitle()
{
  ...
  if (CurrentMenu == 1)               // Main menu
  {
    ...
    TextMid(wscreen / 2.0, hscreen - linenumber * linepitch, MenuTitle[CurrentMenu], &font_dejavu_sans_28, 255, 255, 255, 0, 0, 0);
  }
  else                               // All others
  {
    //setForeColour(255, 255, 255);    // White text
    //setBackColour(0, 0, 0);          // on Black
    TextMid(wscreen / 2.0, hscreen - linenumber * linepitch, MenuTitle[CurrentMenu], &font_dejavu_sans_28, 0, 0, 0, 255, 255, 255);
...
void MsgBox(char *message)
{
  ...
  clearScreen(0, 0, 0);
  TextMid(wscreen / 2, hscreen /2, message, font_ptr, 0, 0, 0, 255, 255, 255);
  TextMid(wscreen / 2, 20, "Touch Screen to Continue", font_ptr, 0, 0, 0, 255, 255, 255);
  UpdateWeb();
  publish();
...
void MsgBox2(char *message1, char *message2)
{
  ...
  clearScreen(0, 0, 0);
  TextMid(wscreen / 2, hscreen / 2 + linepitch, message1, font_ptr, 0, 0, 0, 255, 255, 255);
  TextMid(wscreen / 2, hscreen / 2 - linepitch, message2, font_ptr, 0, 0, 0, 255, 255, 255);
  TextMid(wscreen / 2, 20, "Touch Screen to Continue", font_ptr, 0, 0, 0, 255, 255, 255);
  UpdateWeb();
  publish();
...
void ChangePlutoIP()
{
  ...
  SetConfigParam(PATH_SCONFIG, "plutoip", KeyboardReturn);
    ...
    printf("x=%d y=%d\n", rawX, rawY);
      ...
      if (CurrentMenu == 1)  // Main Menu
      {
        ...
        case 4 :                      // Exit
          ...
          clearScreen(0, 0, 0);
          cleanexit(207); //exit to Portsdown
          break;
...
{
  int button = 0;
  strcpy(MenuTitle[1], "KeyLimePi Signal Generator Main Menu"); 
...
void Define_Menu11()  // Control Panel
{
  ...
  strcpy(MenuTitle[11], "Signal Generator Control Panel");
