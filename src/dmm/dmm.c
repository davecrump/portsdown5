...
#define PATH_DMMCONFIG "/home/pi/portsdown/configs/dmm_config.txt"
...
uint8_t SerialInGPIO = 24;     // Serial In   pin 18 BCM 24
uint8_t LoadSwitchGPIO = 4;    // Load Switch pin 7  BCM 4
...
void ReadSavedParams()
{
  ...
  strcpy(response, "18");
  GetConfigParam(PATH_DMMCONFIG, "serialinpin", response);
  SerialInGPIO = PinToBroadcom(atoi(response));

  strcpy(response, "7");
  GetConfigParam(PATH_DMMCONFIG, "loadswitchpin", response);
  LoadSwitchGPIO = PinToBroadcom(atoi(response));
...
void setGPIOs()
{
  char RequestText[64];
  char InitText[63];
  uint8_t newSerialInGPIO;
  uint8_t newLoadSwitchGPIO;
  ...
  // Set the serial in pin

  // Define request string
  strcpy(RequestText, "Enter the physical pin no for DMM serial (default 18)");

  // Define initial value
  snprintf(InitText, 10, "%d", BroadcomToPin(SerialInGPIO));
 
  // Ask for the new value
  do
  {
    Keyboard(RequestText, InitText, 10);
    newSerialInGPIO = PinToBroadcom(atoi(KeyboardReturn));
  }
  while ((newSerialInGPIO < 0) || (newSerialInGPIO > 40));

  SerialInGPIO = newSerialInGPIO;

  SetConfigParam(PATH_DMMCONFIG, "serialinpin", KeyboardReturn);
  printf("New serial in GPIO set to physical pin %s, Broadcom %d\n", KeyboardReturn, SerialInGPIO);

  // And Set the load switch pin

//uint8_t LoadSwitchGPIO = 4;    // Load Switch pin 7  BCM 4

  // Define request string
  strcpy(RequestText, "Enter the physical pin no for the load switch (default 7)");

  // Define initial value
  snprintf(InitText, 10, "%d", BroadcomToPin(LoadSwitchGPIO));
 
  // Ask for the new value
  do
  {
    Keyboard(RequestText, InitText, 10);
    newLoadSwitchGPIO = PinToBroadcom(atoi(KeyboardReturn));
  }
  while ((newLoadSwitchGPIO < 0) || (newLoadSwitchGPIO > 40));

  LoadSwitchGPIO = newLoadSwitchGPIO;

  SetConfigParam(PATH_DMMCONFIG, "loadswitchpin", KeyboardReturn);
  printf("New load switch GPIO set to physical pin %s, Broadcom %d\n", KeyboardReturn, LoadSwitchGPIO);
...
void Define_Menu4()  // DMM System
{
  ...
  AddButtonStatus(button, "Set GPIO^Pins", &Blue);
...
void DrawYaxisLabels()
{
  ...
  Text(30, 462, y_label, font_ptr, 0, 0, 0, 255, 255, 255);

  snprintf( y_label, 14, "%.4g", PlotBase + 3 * (PlotMax - PlotBase) / 4);
  Text(30, 366, y_label, font_ptr, 0, 0, 0, 255, 255, 255);

  snprintf( y_label, 14, "%.4g", PlotBase + 2 * (PlotMax - PlotBase) / 4);
  Text(30, 266, y_label, font_ptr, 0, 0, 0, 255, 255, 255);

  snprintf( y_label, 14, "%.4g", PlotBase + 1 * (PlotMax - PlotBase) / 4);
  Text(30, 166, y_label, font_ptr, 0, 0, 0, 255, 255, 255);

  snprintf( y_label, 14, "%.4g", PlotBase);
  Text(30,  66, y_label, font_ptr, 0, 0, 0, 255, 255, 255);
}
...
void DrawBaselineText()
{
  ...
  Text(100, BaselineTexty, BaselineText, &font_dejavu_sans_22, 0, 0, 0, 255, 255, 255);
}
...
void *serial_listener(void * arg)
{
  ...
  while (app_exit == false)
  {
    ...
    if ((digitalRead(SerialInGPIO) == space_level) && (awaiting_start))
    {
      ...
      if (digitalRead(SerialInGPIO) == space_level) // still in start bit so progress
      {
        ...
        for (m = 1; m <= 8; m++)  // wait then read each bit
        {
          ...
          if (digitalRead(SerialInGPIO) == mark_level)
          {
            bit[m] = 1;
          }
          else
   ...
   usleep(10000);
    ...
    if (strcmp(DMMMode, "numeric") == 0)                 ///////////////////////  DMM Large Numbers
    {
      ...
      if ((previous_MeterReading > 9999.0) && (BasicMeterReading < -9999.0)) // No valid data yet
      {
        ...
        {
          LargeText(wscreen * 1 / 40, 200 , 2, "No Data", &font_dejavu_sans_36, 0, 0, 0, 255, 255, 255);
          UpdateWeb();
          publish();
      ...
      else
      {
        if ((MeterReading != previous_MeterReading) || (previous_stale_data != stale_data))
        {
          ...
          if (stale_data == true)
          {
            ...
            LargeText(wscreen * 1 / 40, 200 , 1, MeterReadingString, &font_dejavu_sans_72, 0, 0, 0, 255, 255, 255);
          }
          else
          {
            LargeText(wscreen * 1 / 40, 200 , 2, MeterReadingString, &font_dejavu_sans_72, 0, 0, 0, 255, 255, 255);
          }
          publish();
          UpdateWeb();
